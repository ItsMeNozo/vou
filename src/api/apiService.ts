...
export const getVouchersOfUser = async (username: string): Promise<any> => {
  try {
    const response = await api.get(`/sale-events/any/vouchers/user/${username}`);
 ...
 * @returns {Promise<any>}
 */
export const getVoucherDetails = async (eventId: string, voucherId: string): Promise<any> => {
  try {
    const response = await api.get(`/sale-events/${eventId}/vouchers/${voucherId}`);
...
export const getNotificationsByUsernameAndType = async (username: string): Promise<any> => {
  try {
    const type = 'vouFE';
...
export const markNotificationAsRead = async (notificationId: string): Promise<any> => {
  try {
    const response = await axios.patch(`${API_GATEWAY_URL}/notifications/${notificationId}/read`);
    return response.data;
  } catch (error) {
